...
const DigitalTwin = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const formattedData = {
        ...data,
        trajectory: data.trajectory.map((d: any) => ({ ...d, yearLabel: `Year ${d.year}` })),
      };
      ...
      setErrorMsg(
        err instanceof MissingApiKeyError
          ? err.message
          : (err instanceof Error ? err.message : "Failed to generate twin."),
      );
    } finally {
      ...
      return (
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.8rem" }}>
            <p style={{ color: "#ff4d4d", margin: 0, display: "flex", alignItems: "center", gap: "0.3rem" }}>
              <Heart size={14} /> Cardiac: {dataPoint.cardiacRisk}%
            </p>
            <p style={{ color: "#3b82f6", margin: 0, display: "flex", alignItems: "center", gap: "0.3rem" }}>
              <Activity size={14} /> Metabolic: {dataPoint.metabolicRisk}%
            </p>
            <p style={{ color: "#a855f7", margin: 0, display: "flex", alignItems: "center", gap: "0.3rem" }}>
              <Brain size={14} /> Neuro: {dataPoint.neuroRisk}%
            </p>
          </div>
          <p style={{ color: "var(--text-muted)", fontSize: "0.9rem", marginTop: "1rem", fontStyle: "italic" }}>
            "{dataPoint.event}"
          </p>
        </div>
      );
  ...
  const renderOrganBar = (name: string, score: number, color: string, icon: React.ReactNode) => (
  ...
  return (
    <div className="module-container" style={{ display: "flex", gap: "2rem" }}>
      <div className="glass-panel" style={{ flex: "0 0 35%", display: "flex", flexDirection: "column" }}>
        ...
        >
          ...
          <div>
            ...
            <textarea
              placeholder="e.g. Hypertension, Metformin 500mg"
          ...
          <div>
            ...
            <textarea
              placeholder="e.g. Smokes 10/day, sedantry job, high stress"
          ...
          <button
            ...
            style={{
              ...
              fontWeight: "bold",
              fontSize: "1.1rem",
              cursor: isGenerating || !formData.age ? "not-allowed" : "pointer",
              display: "flex",
              alignItems: "center",
          ...
          >
            {isGenerating ? (
              <>
                <Loader2 className="animate-spin" size={20} /> SYNTHESIZING TWIN...
              </>
            ) : (
              <>
                <UserPlus size={20} /> GENERATE DIGITAL TWIN
              </>
            )}
          </button>

          {errorMsg && (
            <p style={{ color: "var(--status-red)", textAlign: "center", margin: 0, marginTop: "1rem" }}>
              {errorMsg}
            </p>
          )}
        </div>
      ...
      <div
        ...
        style={{ flex: 1, display: "flex", flexDirection: "column", padding: "1.5rem", background: "transparent", overflowY: "auto" }}
      >
        ...
        {isGenerating && (
          <div style={{ margin: "auto", textAlign: "center", color: "var(--text-muted)", animation: "pulse 1.5s infinite" }}>
            ...
            <p style={{ color: "#00f0ff", letterSpacing: "2px" }}>
              SIMULATING 80,000 PHYSIOLOGICAL VARIABLES...
            </p>
          </div>
        )}

        {trajectoryData && (
          <div style={{ display: "flex", flexDirection: "column", gap: "2rem", animation: "fadeIn 1s ease-out" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem" }}>
              ...
              >
                ...
                <h2 style={{ margin: 0, color: "#fff", fontSize: "2.5rem" }}>
                  {trajectoryData.biologicalAge}
                  <span style={{ fontSize: "1rem", color: "var(--text-muted)" }}> yrs</span>
                </h2>
                <p
                  style={{
                    color:
                      trajectoryData.biologicalAge > parseInt(formData.age) ? "#ff4d4d" : "#10b981",
                    margin: "0.5rem 0 0 0",
                    fontSize: "0.8rem",
                  }}
                >
                  {trajectoryData.biologicalAge > parseInt(formData.age)
                    ? `+${trajectoryData.biologicalAge - parseInt(formData.age)} years accelerated aging`
                    : `-${parseInt(formData.age) - trajectoryData.biologicalAge} years decelerated aging`}
                </p>
              </div>
              <div
                style={{
              ...
              >
                ...
                <h2 style={{ margin: 0, color: "#fff", fontSize: "2.5rem" }}>
                  {(
                    ...
                    ((parseFloat(formData.height) / 100) * (parseFloat(formData.height) / 100))
                  ).toFixed(1)}
                </h2>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "2rem" }}>
              ...
              >
                ...
                >
                  <Stethoscope size={20} color="#00f0ff" /> Organ Health Index
                </h3>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                >
                  <TrendingUp size={20} color="#00f0ff" /> 10-Year Health Trajectory
                </h3>
            ...
            <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
              ...
              >
                <h4 style={{ color: "#ff4d4d", marginTop: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  <AlertTriangle size={18} /> Critical AI Warning
                </h4>
              ...
              >
                <h4 style={{ color: "#00f0ff", marginTop: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  <Info size={18} /> Preventative Protocol
                </h4>
