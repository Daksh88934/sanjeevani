...
/**
 * Converts a remote image URL to a base64 data URL via a canvas, so the saved
 * report is self-contained (Cloudinary has been removed). Falls back to the
 * original URL if CORS prevents canvas readout.
 */
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        // Canvas tainted — fall back to the remote URL.
        resolve(url);
      }
    };
...
const ArtTherapy = () => {
  ...
  const handleSaveReport = async () => {
    ...
    setIsSaving(true);
    try {
      // Cloudinary removed — embed the image as a base64 data URL.
      const storedImage = await urlToBase64(imageUrl);
      await saveReportToDB("ArtTherapy", patientId, { ...aiReport, imageUrl: storedImage });
  ...
  const generateArt = async () => {
    ...
    try {
      ...
      // 2. Generate Image via free Pollinations API (no key required).
      const finalUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(
        prompt + " hyper-realistic, 8k resolution, cinematic, photorealistic, highly detailed",
      )}?width=1024&height=1024&nologo=true&model=flux&seed=${Math.floor(Math.random() * 1000)}`;
      ...
      img.onerror = () => {
        setErrorMsg("Failed to render artwork.");
        setIsGenerating(false);
      };
    } catch (err) {
      console.error(err);
      setErrorMsg(
        err instanceof MissingApiKeyError
          ? err.message
          : "Failed to generate therapy session. Please try again.",
      );
      setIsGenerating(false);
    }
  };
  ...
  return (
    <div className="module-container" style={{ display: "flex", gap: "2rem" }}>
      <div
        ...
        style={{ flex: "0 0 40%", display: "flex", flexDirection: "column", overflowY: "auto", paddingRight: "1rem" }}
      >
        <div
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          ...
          >
            ...
            >
              {isGenerating ? (
                <>
                  <Loader2 className="animate-spin" size={20} /> ANALYZING PSYCHE & PAINTING...
                </>
              ) : (
                <>
                  <Sparkles size={20} /> INITIATE THERAPY SESSION
                </>
              )}
            </button>
            {errorMsg && (
              <p style={{ color: "var(--status-red)", textAlign: "center", marginTop: "1rem", marginBottom: 0 }}>
                {errorMsg}
              </p>
            )}
          </div>
          ...
          {aiReport && (
            <div style={{ animation: "fadeIn 0.5s ease-out", display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              >
                ...
                >
                  <Brain size={18} /> Psychological Insight
                </h4>
              ...
              >
                ...
                >
                  <Heart size={18} /> Therapeutic Recommendation
                </h4>
              ...
              {aiReport.colorPalette && (
                ...
                >
                  ...
                  >
                    <Palette size={16} /> Healing Color Palette
                  </h4>
                  <div style={{ display: "flex", gap: "1rem" }}>
                    {aiReport.colorPalette.map((color: string, idx: number) => (
                      <div
                        key={idx}
                        style={{
                          flex: 1,
      ...
      <div
        className="glass-panel"
        style={{ flex: 1, display: "flex", flexDirection: "column", padding: "0", background: "transparent" }}
      >
        <div
          className="section-header"
          style={{ padding: "1.5rem", borderBottom: "1px solid var(--glass-border)", margin: 0 }}
        >
          <h2 style={{ fontSize: "1.2rem", margin: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <Palette size={20} color="#db2777" /> AI Emotional Canvas
          </h2>
        </div>
